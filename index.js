...
const uri = `mongodb+srv://${process.env.DB_NAME}:${process.env.DB_PASS}@cluster0.ihnyz1z.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const paymentsCollection = client.db("yoga").collection("payments");
    const enrolledCollection = client.db("yoga").collection("enrolled_classes");
    ...
    app.get("/popular-classes", async (req, res) => {
      const result = await classesCollection
        .find({ status: "Approved" }, { sort: { enrolled: -1 } })
        .limit(6)
        .toArray();
      res.send(result);
    })
